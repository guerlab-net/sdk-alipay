...
public class AlipayMarketingCampaignRuleCrowdCreateModel extends AlipayObject {
     ...
     * 
     * 具体参数参考如下： 1）消费能力：pubsrv_consume_level 操作符：(EQ、IN) 枚举值：高、中、低
     * 2）性别：pubsrv_gender 操作符：(EQ) 枚举值：1男，2女，0未知 3）年龄区间：pubsrv_age操作符：(EQ、IN)
     * 枚举值见后面年龄区间标签枚举定义 4）职业：pubsrv_occupation操作符：(EQ) 枚举值：白领、大学生、未知
     ...
     * 11)达人偏好：pubsrv_preference操作符：(IN)
     * 多值列，枚举值：travel=旅游;video=影视;game=游戏;music=音乐;photography=摄影;pet=宠物;sports=运动;digital=数码
     * 
     * 年龄区间标签枚举 枚举值: 1-11 user_age &gt;=17 then 1 [18,20] then 2 [21,25] then 3
     * [26,30] then 4 [31,35] then 5 [36,40] then 6 [41,45] then 7 [46,50] then
    ...
    public void setMdatacrowdsource(
            String mdatacrowdsource) {
