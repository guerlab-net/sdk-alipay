...
public class AlipayMarketingCampaignCashCreateModel extends AlipayObject {
    ...
    /**
     * 商户打款后的跳转链接，从支付宝收银台打款成功后的跳转链接。不填时，打款后停留在支付宝支付成功页。商户实际跳转会自动添加crowdNo作为跳转参数。示例:
     * http://www.yourhomepage.com?crowdNo=crowdNo
     */
    ...
    /**
     * 活动开始时间,必须大于活动创建的时间. (1) 填固定时间:2016-08-10 22:28:30, 基本格式:yyyy-MM-dd
     * HH:mm:ss (2) 填字符串NowTime
     */
    @ApiField("start_time")
    private String startTime;

    /**
     * 活动发放的现金总金额,最小金额1.00元,最大金额10000000.00元。每个红包的最大金额不允许超过200元,最小金额不得低于0.20元。
     * 实际的金额限制可能会根据业务进行动态调整。
     */
    @ApiField("total_money")
    private String totalMoney;

    /**
     * 红包发放个数，最小1个,最大10000000个。 但不同的发放形式（即prize_type）会使得含义不同： (1)
     * 若prize_type选择为固定金额，每个用户领取的红包金额为total_money除以total_num得到固定金额。 (2)
     * 若prize_type选择为随机金额，每个用户领取的红包金额为total_money除以total_num得到的平均金额值的0.5~1.5倍。由于金额是随机的，在红包金额全部被领取完时，有可能total_num有所剩余、或者大于设置值的情况。
     */
    @ApiField("total_num")
