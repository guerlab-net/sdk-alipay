...
public class AlipayOfflineProviderUseractionRecordModel extends AlipayObject {
    ...
    /**
     * 当前支持类型如下： 1、order_dishes(上传用户菜单) 2、order_num(餐厅排号)
     * 3、order_book_create(餐厅预定) 4、order_pan(泛行业订单上传)
     */
    ...
    /**
     * 该字段建议填写。值定义：alipay、weixin、other、isv; 值意义：alipay：支付宝；weixin：微信；isv：isv
     * 自己的中端系统，other：其他;当前订单的创建来源，比如支付宝扫码创建或微信扫码创建或通过自己的系统用户点菜后创建，则传入对应英文。
     */
    ...
    /**
     * 传入店铺关联关系。标记当前接口涉及到的店铺信息，同时如果传入的数据在口碑不存在，口碑会建立一条shop_id+ outer_id+
     * type的关联数据
     */
    ...
    public void setActionOuterId(
            String actionOuterId) {
    ...
    public void setOuterShopDo(
            OuterShopDO outerShopDo) {
